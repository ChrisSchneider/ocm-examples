...
REGISTRY=registry.internal:10500
S3_URL=http://registry.internal:10900
BUCKET=ocm-examples
...
ocm add cv --repository "$REGISTRY" --constructor examples/1-zot-registry.yml

step "adding 2-known-vulnerabilities"
ocm add cv --skip-reference-digest-processing --repository "$REGISTRY" --constructor examples/2-known-vulnerabilities.yml --config .ocmconfig

step "adding 3-scan-control-labels"
ocm add cv --repository "$REGISTRY" --constructor examples/3-scan-control-labels.yml

step "adding 9-all"
ocm add cv --repository "$REGISTRY" --constructor examples/9-all.yml

log "done — components in $REGISTRY:"
ocm get cv --repo "$REGISTRY"
